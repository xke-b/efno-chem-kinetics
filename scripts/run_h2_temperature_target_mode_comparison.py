#!/usr/bin/env python3
"""Compare temperature target parameterizations on the H2 holdout benchmark."""
...
EXP_DIR = ROOT / "artifacts" / "experiments" / "h2_temperature_target_mode_comparison"
...
CASES = {
    ...
    "supervised_Tnext_25ep": {
        "trainer_name": "supervised_physics",
        "params": {"target_mode": "temperature_next_species"},
    },
...
def train_model(name: str, trainer_name: str, trainer_params: dict) -> Path:
    ckpt = ROOT / "data" / f"h2_holdout_mlp_{name}.pt"
    train_py = f'''
...
def main() -> None:
    ...
    for name, cfg in CASES.items():
        ckpt = train_model(name, cfg["trainer_name"], cfg["params"])
