#!/usr/bin/env python3
"""Compare DeepFlame oneD C2H4 HDF5 sample against canonical and stock active states."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--oned-h5', required=True)
    p.add_argument('--canonical-dataset', required=True)
    p.add_argument('--canonical-metadata', required=True)
...
def oned_h5_summary(h5_path: Path) -> dict:
    with h5py.File(h5_path, 'r') as h5:
        species_names = [str(x) for x in h5.attrs['species_names']]
        arrays = []
        for key in sorted(h5['scalar_fields'].keys(), key=float):
            arrays.append(h5['scalar_fields'][key][...])
    matrix = np.concatenate(arrays, axis=0)
    return summarize_named_matrix(matrix, species_names)



def canonical_summary(dataset_path: Path, metadata_path: Path) -> dict:
    arr = np.load(dataset_path)
    meta = json.loads(metadata_path.read_text())
    species_names = meta['species_names']
    n_species = len(species_names)
    current = arr[:, : 2 + n_species]
    return summarize_named_matrix(current, ['T', 'P', *species_names])



def plot(payload: dict, out_fig: Path) -> None:
    labels = ['oned_deepflame', 'canonical_cantera', 'stock_active_5e-6']
    ...
    for ax, group in zip(axes.flatten(), species_groups):
        ...
        for i, sp in enumerate(group):
            vals = [
                payload['oned_deepflame']['species'][sp]['frac_ge_threshold'],
    ...
    fig.suptitle('C2H4 oneD DeepFlame sample vs canonical Cantera sample vs stock active-state coverage')
...
def main() -> None:
    args = parse_args()
    payload = {
        'oned_deepflame': oned_h5_summary(Path(args.oned_h5)),
        'canonical_cantera': canonical_summary(Path(args.canonical_dataset), Path(args.canonical_metadata)),
        'stock_active_5e6': case_summary(Path(args.case), args.case_time, args.frozen_temperature),
        'note': 'oneD_deepflame and canonical_cantera summarize current-state manifolds; stock_active_5e6 summarizes active cells from the stock DeepFlame C2H4 case.',
    }
