#!/usr/bin/env python3
"""Compare MLP and FNO1d on the H2 temp+species holdout under fixed seeds."""
...
EXP_DIR = ROOT / "artifacts" / "experiments" / "h2_architecture_comparison_seeded"
...
CASES = {
    ...
    "supervised_fno1d": {
        "model_name": "fno1d",
        "model_params": {"width": 32, "modes": 8, "n_layers": 4},
        "trainer_name": "supervised_physics",
        "trainer_params": {"target_mode": "temperature_species"},
    },
...
def train_model(name: str, cfg: dict, seed: int) -> Path:
    ckpt = ROOT / "data" / f"h2_holdout_{name}_seed{seed}.pt"
    train_py = f'''
...
cfg = TrainingConfig(
    model=ModelConfig(name={json.dumps(cfg['model_name'])}, params={json.dumps(cfg['model_params'])}),
    optimizer=OptimizerConfig(name="adam", lr=1e-3),
    trainer=TrainerConfig(
        name={json.dumps(cfg['trainer_name'])},
        max_epochs={EPOCHS},
        lr_decay_epoch=10,
        lr_decay_factor=0.5,
        batch_size=512,
        params={json.dumps(cfg['trainer_params'])},
    ),
    time_step=1e-7,
    seed={seed},
)
...
def main() -> None:
    ...
    for case_name, cfg in CASES.items():
        case_results = []
        for seed in SEEDS:
            ckpt = train_model(case_name, cfg, seed)
    ...
    summary["ranking_by_one_step_species_mae_mean"] = sorted(ranking, key=lambda x: x["one_step_species_mae_mean"])
