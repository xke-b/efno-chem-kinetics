#!/usr/bin/env python3
"""Fine-tune the early dp100 C2H4 FNO on late dp100 data as a simple curriculum test."""
...
CKPT = ROOT / 'artifacts' / 'models' / 'c2h4_casepair_dp100_early_to_late_curriculum_fno_smoke.pt'
EXPORT_DIR = ROOT / 'artifacts' / 'models' / 'c2h4_casepair_dp100_early_to_late_curriculum_fno_smoke_deepflame_bundle'
SUMMARY = ROOT / 'artifacts' / 'experiments' / 'c2h4_casepair_dp100_early_to_late_curriculum' / 'summary.json'
DT = 1e-7
EPOCHS = 3
SEED = 0
...
def train_curriculum() -> dict:
    ...
    optimizer_cfg = OptimizerConfig(name=cfg['optimizer']['name'], lr=cfg['optimizer']['lr'])
    trainer_cfg = TrainerConfig(
        name=cfg['trainer']['name'],
        max_epochs=EPOCHS,
        lr_decay_epoch=max(1, min(EPOCHS, cfg['trainer']['lr_decay_epoch'])),
    ...
    torch.save(
        {
            ...
            'training_config': {
                ...
                'curriculum_stage': 'late_dp100_finetune_with_early_normalization',
            },
        },
        CKPT,
    )
...
def main() -> None:
    ...
    summary = {
        ...
        'export': export_info,
        'note': 'Simple curriculum test: start from early dp100 checkpoint, then fine-tune on late dp100 data using the early normalization contract.',
    }
