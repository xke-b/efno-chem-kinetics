#!/usr/bin/env python3
"""Train/export the first C2H4 FNO baseline on case-aligned CFD state pairs."""
...
DATA = ROOT / 'data' / 'c2h4_case_pairs_smoke.npy'
META = ROOT / 'data' / 'c2h4_case_pairs_smoke.json'
CKPT = ROOT / 'artifacts' / 'models' / 'c2h4_casepair_fno_smoke.pt'
EXPORT_DIR = ROOT / 'artifacts' / 'models' / 'c2h4_casepair_fno_smoke_deepflame_bundle'
SUMMARY = ROOT / 'artifacts' / 'experiments' / 'c2h4_casepair_fno_smoke_baseline' / 'summary.json'
DT = 1e-7
EPOCHS = 6
SEED = 0
...
def main() -> None:
    ...
    summary = {
        ...
        'seed': SEED,
        'export': export_info,
        'note': 'First case-aligned C2H4 CFD-state-pair FNO baseline; labels include full CFD evolution, not chemistry-only targets.',
    }
