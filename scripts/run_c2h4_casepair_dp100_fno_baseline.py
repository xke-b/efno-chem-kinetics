#!/usr/bin/env python3
"""Train/export the first pressure-filtered C2H4 case-pair FNO baseline."""
...
DATA = ROOT / 'data' / 'c2h4_case_pairs_smoke_dp100.npy'
META = ROOT / 'data' / 'c2h4_case_pairs_smoke_dp100.json'
CKPT = ROOT / 'artifacts' / 'models' / 'c2h4_casepair_dp100_fno_smoke.pt'
EXPORT_DIR = ROOT / 'artifacts' / 'models' / 'c2h4_casepair_dp100_fno_smoke_deepflame_bundle'
SUMMARY = ROOT / 'artifacts' / 'experiments' / 'c2h4_casepair_dp100_fno_smoke_baseline' / 'summary.json'
DT = 1e-7
EPOCHS = 6
SEED = 0
...
def torch_load_jsonish(path: Path) -> dict:
    ...
    return {
        'model_path': str(path),
        'export_metadata': payload.get('export_metadata', {}),
    }
...
def main() -> None:
    ...
    summary = {
        ...
        'seed': SEED,
        'export': export_info,
        'note': 'First pressure-filtered C2H4 CFD-state-pair FNO baseline; labels still include full CFD evolution but restrict to small pressure drift.',
    }
